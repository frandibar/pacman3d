...
instalDir="/etc/Pacman3D/"

echo "Insalling.."

#primero copiamos todo a etc/Pacman3D
...
mv "./img" $instalDir"img"

mv "./wav" $instalDir"wav"

mv "./data" $instalDir"data"

chmod 777 $instalDir"img"

chmod 777 $instalDir"wav"

chmod 777 $instalDir"data"

#ahora generamos el makefile

lineNumber=`grep -n -x "__START__" "$0" | cut -f 1 -d ":"`

startLine=`expr "$lineNumber" + 1`

tail -n +$startLine $0 > makefile
